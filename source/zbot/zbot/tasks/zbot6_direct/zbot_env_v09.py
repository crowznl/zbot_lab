...
class ZbotSEnvCfg(DirectRLEnvCfg):
    ...
    sim: SimulationCfg = SimulationCfg(
        ...
        # disable_contact_processing=True,  # Removed Auto
...
def compute_rewards(
    ...
    # it's OK. stand & walk successfully. change pre_action sin(phi), increase up_velocity_reward
    ...
    total_reward = torch.where(body_states[:, 6, 2] > 0.22,
                               ...
                               2*rew_upward + 2*body_states[:, 6, 9] + 2*body_states[:, 5, 9] + 0.1*rew_symmetry - 2*contact_sum \
