...
class Zbot6SnakeEnvV1Cfg(DirectRLEnvCfg):
    ...
    rhythm_base_amplitude = 0.5
    rhythm_use_triangle_wave = False
    rhythm_move_gain = 1.0
...
class Zbot6SnakeEnvV1(DirectRLEnv):
    ...
    def _compute_intermediate_values(self):
        ...
        # rhythm reference (CPG prior): 0.5 * sin(2*t + 1.3*pi*i)
        raw_wave = self.phase.unsqueeze(1) + self.joint_phase_offsets
        ...
        self.cpg_ref = torch.clamp(self.cpg_ref, -self.cfg.joint_delta_limit, self.cfg.joint_delta_limit)
        self.cpg_ref_processed = self._robot.data.default_joint_pos + self.cpg_ref
