...
class Zbot6SnakeEnvV1Cfg(DirectRLEnvCfg):
    ...
    rhythm_frequency_hz = 0.8
    rhythm_phase_lag = 0.9
    rhythm_orthogonal_phase = 1.5708  # pi/2, for orthogonal connected joints
    rhythm_move_gain = 0.55
...
class Zbot6SnakeEnvV1(DirectRLEnv):
    ...
    def __init__(self, cfg: Zbot6SnakeEnvV1Cfg, render_mode: str | None = None, **kwargs):
        ...
        self.joint_pair_ids = torch.div(joint_ids, 2, rounding_mode="floor").unsqueeze(0)
        self.joint_is_odd = (joint_ids % 2).unsqueeze(0)
        self.joint_pair_sign = torch.where(
            (self.joint_pair_ids % 2) == 0,
            torch.ones_like(self.joint_pair_ids),
            -torch.ones_like(self.joint_pair_ids),
        )
    ...
    def _compute_intermediate_values(self):
        ...
        # rhythm reference (CPG prior) for orthogonal connected joints
        # even joints and odd joints are phase-shifted by pi/2, pair-to-pair has traveling lag.
        pair_phase = self.phase.unsqueeze(1) + self.joint_pair_ids * self.cfg.rhythm_phase_lag
        orth_phase = self.joint_is_odd * self.cfg.rhythm_orthogonal_phase
        wave = torch.sin(pair_phase + orth_phase)
