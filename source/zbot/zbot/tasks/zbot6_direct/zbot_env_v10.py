...
from zbot.assets import ZBOT_D_6S_1_CFG
...
class ZbotSEnvCfg(DirectRLEnvCfg):
    # robot
    robot_cfg: ArticulationCfg = ZBOT_D_6S_1_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class ZbotSEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: ZbotSEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 重复最后一维 12 次
        self.e_origins = self.scene.env_origins.unsqueeze(1).repeat(1, self.cfg.num_body+1, 1)
...
def compute_rewards(
    ...
    rew_upward = body_states[:, 6, 2] + 0.5*body_states[:, 4, 2] + 0.5*body_states[:, 9, 2] - 0.1*torch.ones_like(body_states[:, 6, 2])
    ...
    total_reward = torch.where(body_states[:, 6, 2] > 0.22,
                               10*torch.ones_like(rew_upward) + 10*rew_upward + 1.0*(up_proj-1) + 0.5*rew_symmetry,
                               10*rew_upward + 5.0*body_states[:, 7, 9] + 5.0*body_states[:, 6, 9] + 5.0*body_states[:, 5, 9] + 0.5*rew_symmetry - 10*contact_sum - 0.5*torch.abs(joint_pos[:, 0]) - 0.5*torch.abs(joint_pos[:, 5]))
