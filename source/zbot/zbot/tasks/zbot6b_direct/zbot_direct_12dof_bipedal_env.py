...
from zbot.assets import ZBOT_12S_1_CFG

@configclass
class Zbot12BEnvCfg(DirectRLEnvCfg):
    # robot
    robot: ArticulationCfg = ZBOT_12S_1_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class Zbot12BEnv(DirectRLEnv):
    cfg: Zbot12BEnvCfg

    def __init__(self, cfg: Zbot12BEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_observations(self) -> dict:
        ...
        # self._robot.data.heading_w  # z轴 radius
    ...
    def _reward_feet_forward(self):
        ...
        feet_x_w = math_utils.quat_apply(self.feet_quat_w, axis_x_feet)  # torch.Size([4096, 2, 3])
        # print(self.feet_x_w[0])
        # 目前usd的foot坐标系不在底面, 还有注意b(foot1)的坐标系是Xform并没有旋转
    ...
    def _reward_feet_downward(self):
        ...
        feet_z_w = math_utils.quat_apply(self.feet_quat_w, axis_z_feet)  # torch.Size([4096, 2, 3])
        # print(feet_z_w[0])
        # 目前usd的foot坐标系不在底面, 还有注意b(foot1)的坐标系是Xform并没有旋转
        z_w = torch.tensor([0, 0, 1], device=self.sim.device, dtype=torch.float32).repeat((self.num_envs, 2, 1))  # torch.Size([4096, 2, 3])
    ...
    def _reward_base_upward(self):
        axis_z = torch.tensor([0, 0, 1], device=self.sim.device, dtype=torch.float32).repeat((self.num_envs, 1))
        up_w = math_utils.quat_apply(self.base_quat_w, axis_z)  # torch.Size([4096, 3])
        upward = torch.norm((up_w - axis_z), dim=-1,)
        return upward
