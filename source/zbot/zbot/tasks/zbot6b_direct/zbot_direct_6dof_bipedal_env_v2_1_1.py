...
def my_curriculum(env: ZbotDirectEnvV2V1_1, env_ids: torch.Tensor):
    # Stage 1: 2000~4000 iterations (48000 steps)
    if env.common_step_counter >= (2000 * 24) and env.curriculum_stage == 0:
        # Switch to Step 1 config
        # "heading_err_sum": -3.0 (was -1.0)
        # "step_length": 5.0 (new)
        # "airtime_balance": -15.0 (new)
        # "action_rate": -0.1 (new)
        # "torques": -0.002 (new)
        # "feet_slide": -10.0 (new)
        # Removed: feet_force_diff, feet_force_sum
        
        env.reward_scales["heading_err_sum"] = -3.0 * env.step_dt
        env.reward_scales["step_length"] = 5.0 * env.step_dt
        env.reward_scales["airtime_balance"] = -15.0 * env.step_dt
        env.reward_scales["action_rate"] = -0.1 * env.step_dt
        env.reward_scales["torques"] = -0.002 * env.step_dt
        env.reward_scales["feet_slide"] = -10.0 * env.step_dt
        
        # Remove rewards
        if "feet_force_diff" in env.reward_scales:
            env.reward_scales["feet_force_diff"] = 0.0
        if "feet_force_sum" in env.reward_scales:
            env.reward_scales["feet_force_sum"] = 0.0

        env.curriculum_stage += 1
        print(f"Curriculum Stage 1 activated at step {env.common_step_counter}")

    # Stage 2: 4000~6000 iterations (96000 steps)
    elif env.common_step_counter >= (4000 * 24) and env.curriculum_stage == 1:
        # Switch to Step 2 config
        # "feet_downward": -2.0 (was -1.0)
        # "airtime_sum": 2.0 (new/restored from step0 comment if it was there, but step0 had 10.0 commented out)
        
        env.reward_scales["feet_downward"] = -2.0 * env.step_dt
        env.reward_scales["airtime_sum"] = 2.0 * env.step_dt

        env.curriculum_stage += 1
        print(f"Curriculum Stage 2 activated at step {env.common_step_counter}")

    # Stage 3: 6000~8000 iterations (144000 steps)
    elif env.common_step_counter >= (6000 * 24) and env.curriculum_stage == 2:
        # Switch to Step 3 config
        # "heading_err_sum": -5.0 (was -3.0)
        # "airtime_sum": 3.0 (was 2.0)
        
        env.reward_scales["heading_err_sum"] = -5.0 * env.step_dt
        env.reward_scales["airtime_sum"] = 3.0 * env.step_dt

        env.curriculum_stage += 1
        print(f"Curriculum Stage 3 activated at step {env.common_step_counter}")
...
@configclass
class ZbotDirectEnvCfgV2V1_1(DirectRLEnvCfg):
    # robot
    ...
    reward_cfg = {
        "reward_scales": {
            ...
            "feet_force_diff": 0.5,
            "feet_force_sum": -0.1,
            # Placeholder for rewards introduced later, init to 0
            "step_length": 0.0,
            "airtime_balance": 0.0,
            "action_rate": 0.0,
            "torques": 0.0,
...
class ZbotDirectEnvV2V1_1(DirectRLEnv):
    cfg: ZbotDirectEnvCfgV2V1_1

    def __init__(self, cfg: ZbotDirectEnvCfgV2V1_1, render_mode: str | None = None, **kwargs):
