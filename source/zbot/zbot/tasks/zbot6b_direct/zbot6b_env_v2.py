...
class ZbotBEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: ZbotBEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # self._commands = torch.tensor([-0.1], dtype=torch.float32, device=self.sim.device).repeat((self.num_envs, 1))
        ...
        self.last_joint_vel = torch.zeros_like(self.zbots.data.joint_vel[:, self._joint_idx])
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # print(self._commands[0, 0])
        # print(self.body_states[0, 3, 8])
        ...
        rew_dof_acc = torch.sum(torch.square((self.last_joint_vel - self.joint_vel) / self.step_dt), dim=1)
...
def compute_rewards(
    ...
    step_dt: float,
    rew_dof_vel: torch.Tensor,
    rew_dof_acc: torch.Tensor
):
    ...
    y_vel_error = torch.square(commands[:, 0] - body_states[:, 3, 8])
    # print(y_vel_error)
